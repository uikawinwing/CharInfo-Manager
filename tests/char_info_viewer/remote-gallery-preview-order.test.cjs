...
function pack(title, source) {
  return {
    ...
    version: 1,
    packId: 'profile-editor-preview-test',
...
test('scoped Profile Editor Remote Gallery requests resolve in invocation order', async t => {
  clearRemoteGalleryCache();
  ...
  const scope = 'profile-editor-viewer';
...
test('Remote Gallery requests carry an abort signal and a finite timeout policy', async t => {
  clearRemoteGalleryCache();
  const originalFetch = global.fetch;
  let seenSignal = null;
