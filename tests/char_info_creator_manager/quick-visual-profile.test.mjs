...
const {
  buildQuickVisualWorldbookEntry,
  quickVisualEntryName,
  readQuickVisualProfileFromChatVariables,
  saveQuickVisualProfileToCurrentChatWorldbook,
} = require('../../src/char_info_creator_manager/quickVisualProfile.ts');
const { extractManagedEjsBlock } = require('../../src/char_info_shared/characterVisualProfile.ts');

test('快速视觉从当前聊天 CharInfo 读取现有图片配置，不复制角色状态资料', () => {
  const profile = readQuickVisualProfileFromChatVariables('奥琳科・乌尔芬', {
    char_info: {
      profiles: {
        '奥琳科・乌尔芬': {
          schema_version: 2,
...
test('快速视觉世界书条目只保存 managed EJS，并以角色姓名作为关键词', () => {
  const entry = buildQuickVisualWorldbookEntry({
  ...
  assert.equal(quickVisualEntryName('奥琳科・乌尔芬'), '[CharInfo][视觉] 奥琳科・乌尔芬');
...
test('快速视觉第一次建立条目，之后保存会更新同一条目而不是重复新增', async () => {
  let entries = [];
  let nextUid = 1;
  ...
  try {
    const first = await saveQuickVisualProfileToCurrentChatWorldbook(makeProfile('https://img.example/one.webp'));
    assert.equal(first.created, true);
    assert.equal(entries.length, 1);
    assert.equal(entries[0].uid, first.entryUid);

    const second = await saveQuickVisualProfileToCurrentChatWorldbook(makeProfile('https://img.example/two.webp'));
