...
test('管理器 WebM 默认暂停，只在 hover 或触屏操作时播放，并保证单实例播放', () => {
  ...
  assert.match(galleryStepSource, /@click="moveImageSource\(image, sourceIndex, 1\)"/);
  assert.match(galleryStepSource, /\[CharInfo\]\[ImageFallback\]\[ProfileEditor\]/);
});

test('视频主立绘不会被写成头像，角色档案编辑器会继续使用可用静态图片', () => {
  const galleryStepSource = readFileSync(
    new URL('../../src/char_info_profile_editor/components/GalleryStep.vue', import.meta.url),
    'utf8',
  );
  const galleryEditorSource = readFileSync(
    new URL('../../src/char_info_profile_editor/galleryEditor.ts', import.meta.url),
    'utf8',
  );
...
test('角色档案编辑器在初始化资料前先建立编辑器状态', () => {
  ...
  for (const declaration of requiredDeclarations) {
    ...
    assert.notEqual(declarationIndex, -1, `缺少 角色档案编辑器状态：${declaration}`);
...
test('世界书远程图库预览限制为六并发，不再一次打出全部角色请求', () => {
  const librarySource = readFileSync(
    new URL('../../src/char_info_viewer_runtime/WorldbookCharacterLibrary.vue', import.meta.url),
    'utf8',
  );
