...
import { buildProfileEditorViewerVisualOverride } from '../../src/char_info_profile_editor/viewerPreview.ts';
import {
  inspectManagedBlock,
  MANAGED_BLOCK_END,
  MANAGED_BLOCK_START,
...
test('可迁移 draft 可以直接进入现有 角色卡预览 Preview 视觉 override', () => {
  const source = `setLocalVar('char_info_visuals["傲雪"]', {
    url: 'https://files.catbox.moe/main.webp',
  ...
  const inspection = assertImportable(inspectLegacyVisualProfile(source, '傲雪'));
  const override = buildProfileEditorViewerVisualOverride(inspection.profile);
...
test('角色档案编辑器对 importable legacy 解除写入阻塞，并启用升级保存文案', async () => {
  const profileEditorSource = await readFile(new URL('../../src/char_info_profile_editor/App.vue', import.meta.url), 'utf8');

  assert.match(profileEditorSource, /inspectLegacyVisualProfile/);
  assert.match(profileEditorSource, /legacyVisualInspection\.value\.state === 'importable'/);
  assert.match(profileEditorSource, /replaceProfile\(legacyInspection\.profile\)/);
  assert.match(profileEditorSource, /已从旧版 \$\{legacyInspection\.sourceRoot\} 安全预填；当前世界书尚未修改/);
