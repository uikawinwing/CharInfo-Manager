...
test('角色档案编辑器世界书选择只使用当前角色相关、全局与当前聊天世界书', () => {
  assert.match(profileEditorSource, /getGlobalWorldbookNames\(\)/u);
  assert.match(profileEditorSource, /getChatWorldbookName\('current'\)/u);
  assert.match(profileEditorSource, /buildCurrentWorldbookList\(binding, globalWorldbooks, chatWorldbook\)/u);
});
