...
test('角色档案编辑器 Step 2 按角色资料、展示文案、角色故事、作者署名组织字段', () => {
  ...
  assert.doesNotMatch(stepTwo, /背景故事/u, '角色档案编辑器 UI 不应暴露生成的背景故事字段');
  assert.doesNotMatch(stepTwo, /Viewer|story_sections|编辑器内部排序 ID/u, '角色档案编辑器可见文案不应暴露实现术语');
});

test('角色档案编辑器 Step 4 由独立 GalleryStep 管理唯一用途、批量显示范围与用途标签', () => {
  ...
  assert.match(galleryStepSource, /显示于角色卡/u);
...
test('角色档案编辑器编辑态 story id 不会进入序列化 metadata', () => {
...
test('角色档案编辑器 metadata/story UI 在 mobile 与 force-mobile 下保持单列和可触控操作', () => {
  assert.match(
    appSource,
