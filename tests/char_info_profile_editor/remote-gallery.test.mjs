...
import { parseRemoteGalleryPayload } from '../../src/char_info_shared/remoteGallery.ts';

test('char-info-gallery-pack v1 保留图片、视频与可选 thumbnail', () => {
  const payload = parseRemoteGalleryPayload({
...
test('远程图库拒绝旧 visual pack 与非 HTTPS 媒体', () => {
  assert.throws(
    () =>
      parseRemoteGalleryPayload({
