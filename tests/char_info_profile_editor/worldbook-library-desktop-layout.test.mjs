...
test('世界书角色库与角色详情都由 Viewer 提供，详情关闭后仍返回角色库', () => {
...
test('手机角色库使用固定宿主坐标系、安全区底栏，并保留来源、世界书、搜索与筛选入口', () => {
  ...
  assert.doesNotMatch(librarySource, /mobile-library-back-to-top\.menu-open/u);
  ...
  assert.match(librarySource, /watch\(selectedEditableWorldbookName, worldbookName => emit\('selectionChanged', worldbookName\), \{ immediate: true \}\)/u);
  assert.doesNotMatch(librarySource, /mobileMoreOpen|editLibrary|mobile-library-more|manager-view-switch/u);
  assert.match(librarySource, /aria-label="重新读取角色库"[\s\S]*?@click="loadWorldbooks"[\s\S]*?<span>刷新<\/span>/u);
