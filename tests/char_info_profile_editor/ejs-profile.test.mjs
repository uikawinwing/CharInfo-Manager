...
import {
  ...
  validateProfile,
} from '../../src/char_info_shared/characterProfile.ts';
...
test('远程图库 URL 允许不配置本地立绘，并经过 managed EJS round-trip', () => {
  const remoteUrl = 'https://img.example.test/api/public/gallery/uika/elfa1';
  const remoteOnlyProfile = {
    ...createEmptyProfile('远程角色'),
    remoteGalleryUrl: remoteUrl,
  };

  assert.deepEqual(validateProfile(remoteOnlyProfile), []);
  const block = buildManagedEjsBlock(remoteOnlyProfile);
  assert.match(block, /gallery_pack_url: profile\.remoteGalleryUrl/);
  assert.doesNotMatch(block, /visual_remote_url|gallery_extension/);
  assert.ok(block.includes(remoteUrl));

  const inspection = inspectManagedBlock(block);
  assert.equal(inspection.state, 'valid');
  assert.equal(inspection.profile.remoteGalleryUrl, remoteUrl);
  assert.deepEqual(inspection.profile.gallery, [{ title: '主立绘', sources: [] }]);

  assert.match(
    validateProfile({ ...remoteOnlyProfile, remoteGalleryUrl: 'http://img.example.test/api/public/gallery/uika/elfa1' }).join('\n'),
    /HTTPS/,
  );
});

test('旧 v2 managed block 的 galleryPackUrl 仍会读成 remoteGalleryUrl', () => {
  const remoteUrl = 'https://img.example.test/api/public/gallery/uika/legacy';
  const currentBlock = buildManagedEjsBlock({
    ...createEmptyProfile('旧远程图库角色'),
    remoteGalleryUrl: remoteUrl,
  });
  const oldNamedBlock = currentBlock.replaceAll('remoteGalleryUrl', 'galleryPackUrl');

  const inspection = inspectManagedBlock(oldNamedBlock);
  assert.equal(inspection.state, 'valid');
  assert.equal(inspection.profile.remoteGalleryUrl, remoteUrl);
});
