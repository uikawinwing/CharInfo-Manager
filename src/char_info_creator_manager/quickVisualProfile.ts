...
export const QUICK_VISUAL_ENTRY_PREFIX = '[CharInfo][视觉] ';
...
export function readQuickVisualProfileFromChatVariables(
  ...
  const profile = normalizeProfile({
    ...
    ...(readString(stored.gallery_pack_url) ? { galleryPackUrl: readString(stored.gallery_pack_url) } : {}),
    gallery,
    ...(isRecord(stored.metadata) ? { metadata: stored.metadata as CharacterProfileMetadata } : {}),
  });

  if (profile.gallery.length === 0 && !profile.galleryPackUrl) {
    profile.gallery = createEmptyProfile(name).gallery;
  }
  return profile;
}

export function quickVisualEntryName(characterName: string): string {
  return `${QUICK_VISUAL_ENTRY_PREFIX}${characterName.trim()}`;
}

export function buildQuickVisualWorldbookEntry(profile: CharacterVisualProfile) {
  const normalized = normalizeProfile(profile);
  const errors = validateProfile(normalized);
  if (errors.length > 0) throw new Error(errors[0]);

  return {
    name: quickVisualEntryName(normalized.characterName),
    enabled: true,
...
export async function saveQuickVisualProfileToCurrentChatWorldbook(
  profile: CharacterVisualProfile,
): Promise<{ worldbookName: string; entryUid: number; created: boolean }> {
  const normalized = normalizeProfile(profile);
  const entryInput = buildQuickVisualWorldbookEntry(normalized);
