...
async function evaluateProfileEditorEjs(code: string, debugEnabled: boolean, when: string): Promise<void> {
...
export async function evaluateManagedEjs(code: string, debugEnabled = false): Promise<void> {
  await evaluateProfileEditorEjs(code, debugEnabled, 'char-info-profile-editor-apply');
}
