<template>
  <div class="gallery-step" :class="{ 'flash-mode': props.flashMode }">
    <p v-if="props.flashMode" class="flash-mode-guidance">
      粘贴图片直链即可。第一张会作为主立绘；第一张可用静态图片会自动用于头像和角色库封面。
    </p>
    <section v-if="!props.flashMode" class="role-panel">
      ...
      <div class="role-grid">
        <label class="field">
          ...
          <small>选择后会成为角色卡第一张；图片和视频都可以。</small>
        </label>
    ...
    <section v-if="!props.flashMode" class="gallery-storage-panel">
      <div class="panel-heading">
        <h3>远程图库</h3>
        <p>填写 ImgBed 的公开图库 API URL 后，角色卡会从该 URL 读取整套图片；下方本地图片只作为断线 fallback。</p>
      </div>
      <label class="field">
        <span class="field-label">远程图库 URL <small>选填</small></span>
        <input
          ...
          @input="emit('update:remoteGalleryUrl', ($event.target as HTMLInputElement).value)"
        />
        <small>只接受 HTTPS 的远程图库协议（兼容格式名：char-info-gallery-pack v1）。留空时使用本地图片；远端读取不会写回聊天变量或世界书。</small>
      </label>
    </section>
    ...
    <div v-if="!props.flashMode" class="batch-toolbar" :class="{ active: batchMode }">
          ...
          <div class="gallery-card-heading">
            <div class="usage-pills" aria-label="图片用途">
              ...
              <span class="usage-pill" :class="{ album: !isViewerVisible(image) }">
                {{ isViewerVisible(image) ? '显示于角色卡' : '仅相册' }}
              </span>
...
function applyBatchVisibility(visible: boolean) {
  ...
  if (!setViewerVisibility(gallery.value, selectedIds.value, visible)) {
    batchMessage.value = '至少要保留一张角色卡主立绘；请少选一张后再设为仅相册。';
    return;
  }
  batchMessage.value = visible ? '已设为显示于角色卡。' : '已设为仅相册。';
}
...
function debugGalleryPreview(image: EditableGalleryImage, event: string, details: Record<string, unknown> = {}) {
  ...
  const sources = resolveGalleryPreviewSources(image);
  console.info('[CharInfo][ImageFallback][ProfileEditor]', {
    event,
...
.gallery-step.flash-mode .gallery-card-heading,
.gallery-step.flash-mode .gallery-fields > .field:first-child,
.gallery-step.flash-mode .source-order-actions,
.gallery-step.flash-mode .add-source-button {
  display: none;
}

.gallery-step.flash-mode .gallery-fields {
