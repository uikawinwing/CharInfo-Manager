...
- 修改前查看同目录已有写法，保持项目风格。
- CharInfo 查看器的版式只有三类，禁止自行创造或混用“普通立绘版”等分类：**普通角色**没有立绘，使用普通无图版式；**Special NPC**有立绘，使用 `special_npc` 版式；**DX**仅限 DX roster 中指定的 4 名角色，使用其独立 DX 版式。路由优先级为 DX，其次有立绘的 Special NPC，最后无立绘的普通角色。
