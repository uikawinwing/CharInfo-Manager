...
> 历史状态文档：最后主要更新于 2026-08-13。
> v0.1.9 之后，本文件不再是当前执行顺序的唯一依据；保留它是为了追溯 v0.1.7 stabilization 与早期 NEXT 决策。
> 当前交接请先阅读 `docs/v0.1.9_handover.md`；下一功能线以 `docs/v0.2.0_patch_plan.md`、仓库根目录 `AGENTS.md` 与 Project Instructions 为准。

## 1. 已确认且不得自行改变的产品分类

CharInfo 角色查看器 只有三种版式，不存在“普通立绘版”“普通带图版”或第四种中间版式。

| 分类 | 判定方式 | 当前范围 |
| --- | --- | --- |
| 普通角色 | 没有有效立绘，且不是可信 DX | 保留现有无图版式；未经 Master 明确要求，不重做布局、页签或视觉 |
| 立绘角色卡 | 不是可信 DX，并且该姓名在 `char_info.profiles` 中拥有有效立绘 | 使用 `special_npc` 版式；当前响应式重设计只属于这一类 |
| DX | roster 固定四名；当前只有前三名可通过可信 DX 占位符与 loader 加载，Siren（瑟涟）待确认启用 | 使用 DX 版式；不能通过姓名、立绘、profile 或 metadata 获得 DX 身份 |

路由优先级固定为：

```text
可信 DX > 受信视觉资料授予的 立绘角色卡 > 普通角色
```

必须遵守：

- `char_info.profiles[姓名]` 是 立绘角色卡 唯一的运行时视觉资料来源；旧 `char_info_visuals` / `char_info.visual(s)` 只由 角色档案编辑器 负责导入迁移。
- `<char_info>` 内直接出现的图片 URL、普通变量占位符或其他图片字段不能授予 立绘角色卡 身份，也不能改变 UI 路由。
- 没有受信视觉资料且不是可信 DX 的角色继续使用普通版；即使正文里出现图片字段，也不能因此升级成 立绘角色卡。
- `char_info.profiles[姓名]` 不是 DX 身份凭证。
- 不使用 `presentation.kind = special` 作为 立绘角色卡 开关。
- 不允许 `presentation.kind = dx`、metadata、姓名匹配或手造 `__dx_character_ref` 授予 DX 身份。
- 不把 立绘角色卡 的六页签、雷达面板或响应式样式套到普通无图版。
...
- 无立绘角色继续使用普通无图版。
- 立绘角色卡 桌面使用左侧立绘、右侧资料和 72px 右侧导航栏。
- 立绘角色卡 手机卡片使用宽度驱动的 `2:3` 固定比例，不依赖 `vh` / `dvh`。
...
- 登神页在 立绘角色卡 手机端使用紧凑样式。
- 面板页使用雷达图、HP / SP / MP 数字和可选状态效果，不使用五张旗帜。
- 立绘角色卡 立绘失败时可以回退到普通无图版。
...
### 3.4 角色查看器 与 角色档案编辑器 模块边界

状态：✅ v0.1.7 已重新合并为单安装脚本；角色查看器 / 角色档案编辑器 内部职责继续分离

- 对用户与发布只保留一个 CharInfo Manager 脚本，避免同时安装、更新和管理 角色查看器 / 角色档案编辑器 两份脚本。
- 源码职责仍保持分离：角色查看器运行时 负责玩家功能，角色档案编辑器只负责编辑和保存角色档案。
- 角色查看器、角色档案编辑器、shared、runtime 文件与函数不能因为重新合包而重新混在一起。
- Editor 只在用户进入编辑流程时 mount，关闭后销毁；合包不代表 Editor 常驻运行。
- v0.1.7 删除只为双脚本通信存在的 host bridge，改为同 bundle 内的窄 角色档案编辑器 API；角色查看器 不直接操作 角色档案编辑器 的 Vue 状态。
- 玩家世界书角色库和条目启停逻辑继续属于 角色查看器运行时，不移回 角色档案编辑器。
...
- 详情页可以查看图库和世界书正文；只有编辑按钮会调用 角色档案编辑器。
...
- ⬜ 角色档案编辑器 自定义故事栏目尚未实现。
- ⬜ 角色查看器 的通用“角色故事”页签尚未实现。

### 4.2 图库功能

- ✅ 角色查看器 玩家详情可以显示图片、视频和备用图床资料。
- ✅ v0.1.7 已补齐 角色卡预览 与 立绘角色卡查看器 的备用源状态机：`error`、长时间 pending 与失败页手动 Retry 都会按 `sources[]` 顺序前进，耗尽后才 wrap；Master 已在 SillyTavern 实机确认 `URL0 error → URL1 try → URL1 loaded`，pending/timeout 路径已有自动回归，仍可在最终验收时补一次现场 timeout 注入。
- ✅ 角色查看器运行时 已停止把 `status.externalGalleries.partners` 自动迁移并写回 `char_info.profiles`；角色查看器 保持只读。旧 status gallery 的显式迁移与兼容输出留给 角色档案编辑器 后续功能。
- ⬜ status gallery 的显式 Import-to-draft、保存时兼容输出及安全移除旧 EJS 属于 角色档案编辑器 UX，默认放到 v0.1.8，不扩大 v0.1.7。
...
### 5.1 角色档案 Metadata 存储与 round-trip

状态：✅ 后续版本已完成（本段只保留历史位置）

当前源码已经使用 `CharacterProfile`，并支持 Metadata 的 normalize / validate / load / save / managed EJS round-trip。Metadata 可包含作者、版本、作者说明、性别、种族与故事栏目。

这里的 Metadata 是角色档案的可选附加资料；**它不参与立绘角色卡资格判定**。立绘角色卡仍要求同名 `char_info.profiles[姓名]` 提供至少一个角色卡可用图片来源。角色档案编辑器的快速模式不要求 Metadata。当前定义以 `docs/terminology-keymap.md` 为准。

### 5.2 自定义角色故事编辑器

状态：⬜

- 角色档案编辑器 Step 2 尚未提供故事栏目新增、编辑、上移、下移和删除。
...
### 5.4 角色档案编辑器 使用真实 角色查看器 预览

状态：🟡 v0.1.7 已完成实现与自动回归，待 SillyTavern 实机打开／关闭与实时草稿更新验收。

角色档案编辑器 现可直接挂载真实 `ViewerApp` 预览未保存 draft；“应用已保存版本到当前聊天”仍是另一条保存后 EJS 工作流，两者不得混为一谈。

已实现：

- 在 角色档案编辑器 内挂载真实 角色查看器 进行完整角色卡预览。
...
- Preview 使用 `v-if` 生命周期，关闭时卸载真实 角色查看器；仍需 SillyTavern 实机重复打开／关闭验收监听器清理。
...
- 在实际 SillyTavern 中逐项检查桌面与手机 立绘角色卡。
...
- 审查当前尚未提交的 DX 门禁、DX 目录、双脚本拆分、角色资料库等现有工作。
...
#### Checkpoint 1 — 角色查看器 / 角色档案编辑器 单脚本 reunification

状态：✅ 源码 / test / lint / production build / 实际 SillyTavern lifecycle 已验证

- 用户只安装和管理一个 CharInfo Manager 脚本。
- 角色查看器、角色档案编辑器、shared、runtime 模块继续分目录、分职责。
- 删除仅为双脚本通信存在的 host bridge，改为同 bundle 内窄 API。
- Editor 只在需要时 mount，关闭后销毁；不能因为合包变成常驻开销。
- 不把角色资料库、世界书启停或 角色查看器 状态逻辑搬进 角色档案编辑器。
- 完成后在实际 SillyTavern 验证 角色查看器 与 Editor 都可打开、关闭并重复使用，再停止。

#### Checkpoint 2 — 路由与只读数据边界

状态：✅ 立绘角色卡 路由白名单 / 角色查看器 legacy gallery 只读边界 / test / lint / production build / 实际 SillyTavern 立绘角色卡 已验证

- DX 只认可信 DX placeholder + loader；不得通过姓名、profile、metadata 或手造字段升级。
- 立绘角色卡 只由 `char_info.profiles[姓名]` 授予；旧视觉变量不再参与 角色查看器 路由。
- `<char_info>` 内直接图片 URL、普通变量占位符及其他旧图片字段一律忽略，不进入 角色查看器 图片数据；若同时存在同名 `char_info.profiles`，角色查看器 继续使用新版 profile，并复用现有解析 warning 样式提示作者清理旧字段。
- 角色查看器运行时 停止把 `status.externalGalleries` 迁移并写回 `char_info.profiles`；角色查看器 保持只读。
- 旧 status gallery 如需兼容，只能作为只读 fallback；显式迁移与兼容输出留给 角色档案编辑器 后续功能。
...
- 处理独立 DX entry 与主 角色查看器 静态 import 两条泄漏路径。
- 加 production regression，检查最终 dist 不含已知 DX identifiers / registry signatures。
- 不修改 DX UI 布局、视觉或角色 presentation。
- 完成后停止。

#### Checkpoint 4 — 角色查看器 玩家 Save 与图片 fallback bug

状态：✅ 图片 fallback 与 立绘角色卡 角色查看器 Save 已完成实现；Save 两种写入、真实成功反馈与 `URL0 error → URL1 loaded` 已完成实机／自动回归。长时间 pending 的现场 timeout 注入统一留到最终跨模式回归。

- 角色查看器 卡片必须保留给玩家使用的 Save 操作；点击后可选择写入 MVU 角色状态或聊天世界书。立绘角色卡 不得因为 side-rail 分支而硬编码隐藏该入口；只读 角色查看器 仍不得显示写入操作。
- 修复 角色卡预览 与 立绘角色卡查看器 的备用图床：`URL0` error 或长时间 pending 时必须可靠尝试 `URL1`；进入“立绘无法加载”状态后，用户点击重试也应切换到下一备用 source（耗尽后再 wrap），不能永久卡回同一失败源。
- 角色档案编辑器 允许作者对同一图片的 `sources[]` 逐条上移／下移，明确保存 URL0、URL1 等默认 fallback 顺序；玩家侧 hostname 优先级仍是独立的本机覆盖层。
- Runtime Settings 增加默认关闭的 Debug 模式；开启后 角色查看器 与从 Runtime 打开的 角色档案编辑器 使用 `[CharInfo][ImageFallback]` Console 日志记录实际尝试 URL、source index、error／timeout、fallback、loaded、all failed 与 retry。
...
状态：✅ v0.1.7 桌面与手机布局、最终跨模式 SillyTavern 回归、角色卡预览 生命周期、图片 fallback timeout 实机验证与 角色资料库清理均已完成；当前只剩 Git 收口。

- Chrome Network / DOM 与源码对照发现：角色静态 Catbox PNG 原本会被 角色查看器 / 角色档案编辑器 改写成 `wsrv.nl` proxy URL，导致无法复用其他脚本／预加载器已缓存的原始 Catbox URL，并增加一次代理回源。v0.1.7 已让角色立绘与 角色卡预览 保留作者原始 URL；DX 明确使用的背景 proxy 不改。
- 主立绘仍保留浏览器 `loading="lazy"`；先只修已确认的 proxy/cache mismatch，不同时修改第二个性能变量。若现场仍明显偏慢，再继续测 lazy-load 时序。
- Current NPC Panel 的心里话覆盖曾丢失 立绘角色卡 内部视觉身份，已修复并在当前 SillyTavern 实机确认千爻保持 立绘角色卡 有图版。
...
- ✅ 角色档案编辑器 保存成功后可“应用已保存版本到当前聊天”：只提取当前条目中通过 `inspectManagedBlock()` 验证的 CharInfo managed EJS，并通过 ST-Prompt-Template `prepareContext()` + `evalTemplate()` 精确执行该片段，再调用 `saveVariables()` 持久化 `setLocalVar()` 的修改；不触发其他世界书条目，随后验证 chat variables 并调用同一 Force Refresh。未保存 draft 的真实 角色查看器 Preview 也已提前至 v0.1.7 实现，最终实机验收见 5.4。
- ✅ v0.1.7-desktop：立绘角色卡 桌面首页／面板布局、右侧分页顺序、五维旗帜比例与数值位置、登场台词空间、Save 两种写入反馈，以及本轮桌面交互均已由 Master 在 SillyTavern 实机确认。
- ✅ v0.1.7-mobile 主要实现：立绘角色卡 手机首页只保留立绘／登场台词／姓名；六项底栏与 Save 操作重新收口；面板承接等级、层级、种族、身份、职业、五维与资源；资料页使用玩家最后看到的立绘静态快照作为半透明 wallpaper，并冻结动图当前帧；技能／持有页完成移动端字号层级、间距、gutter 与背景承托；登神页保持紧凑可滚动布局。首页切换到第二张立绘后进入面板仍沿用第二张 wallpaper 已由实机确认。普通无图版、可信 DX、Editor lifecycle、图片 fallback timeout 注入、Debug Console / Network 仍需在最终跨模式回归统一收口。
...
#### Checkpoint 7 — 角色档案编辑器 自定义故事编辑器

- 只实现 Step 2 的故事栏目编辑。
- 复用 metadata 存储契约。
- 完成后停止。

#### Checkpoint 8 — 角色查看器 自定义角色故事
...
#### Checkpoint 9 — 角色档案编辑器 真实 角色查看器 预览（已提前至 v0.1.7）

- 实现状态与最终实机验收见 5.4；v0.1.8 不再重复实现第二份 Preview。
- 后续这里只处理 Preview 被实际使用后发现的独立缺陷，不扩展成另一套 角色查看器。

#### Checkpoint 10 — Gallery / status compatibility 角色档案编辑器 UX
...
- 用 `presentation.kind = special` 决定 立绘角色卡。
- 存在“普通立绘版”或“普通带图版”。
- 把 立绘角色卡 的六页签、雷达和响应式外壳同步到普通无图版。
- 把普通版、立绘角色卡 和 DX 强行统一为同一套页面模型。
...
- 继续把玩家世界书角色库放在 角色档案编辑器 内。
- 把 角色档案编辑器 的单张图片预览误称为“真实 角色查看器 预览已完成”。
...
- DX、立绘角色卡、普通角色三类路由没有互相泄漏。
- 旧 profile 与旧角色无需迁移即可继续使用。
- 角色档案编辑器 不会在无意中擦除未知或可选资料。
